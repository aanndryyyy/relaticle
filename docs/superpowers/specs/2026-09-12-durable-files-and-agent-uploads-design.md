...
- Stack: Laravel 13.31, Filament 5.8.1, Pest 5.1.4, relaticle/custom-fields 3.9.1, spatie/laravel-medialibrary 11.23.7, laravel/mcp 0.9.5. `filament/spatie-laravel-media-library-plugin` 5.8.1 is installed and serves the workspace logo.
...
- The panel does not wrap saves in a database transaction. Validation that must block a write runs in the `saving` observer hook, before the row exists.
...
| record (`Company`, `People`, `Opportunity`, `Task`, `Note`) | `attachments` | the file behind a `file-upload` value or the inline images of a rich-editor value, keyed to the field by `custom_field_id` |
...
2. Validate. `OwnedUpload` runs in `ValidCustomFields` (REST, MCP, chat) and in the panel component's path check. A `file-upload` value must be a uuid that is pending in the caller's workspace or already owned by this record and field. The `saving` observer hook runs the same check so a race between two saves cannot persist a value that names a file another record claimed.
...
`App\Filament\CustomFields\FileUploadFieldType` swaps the package form component for one built on Filament's `FileUpload` with three closures: `saveUploadedFileUsing` stores a pending row and returns its `uuid`, `getUploadedFileUsing` resolves a uuid to its name, size, type and URL, `deleteUploadedFileUsing` deletes the pending row. Table column and infolist entry render the name as a link.
...
`.ai/rules/file-uploads.md` states the rule. `tests/Arch/ConventionsTest.php` fails on a new `FileUpload::make` or attachment disk configuration outside the two media-backed components and the profile photo form.

## Outside this PR

- Chat gets no upload tool. `file-upload` stays deferred there. `.ai/rules/chat.md` says a field reachable in the panel must be settable from chat; the founder decides whether that gap blocks enabling the type.
